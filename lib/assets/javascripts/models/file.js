(function () {
	...
	var File = Marbles.Model.createClass({
		...
		willInitialize: function () {
			...
			this.on('change:permissions.public', this.handleChangePermissions, this);
			this.on('change:attachments', this.handleChangeAttachments, this);
			this.on('change:entity', this.handleChangeEntity, this);
		},

		parseAttributes: function () {
			this.constructor.__super__.parseAttributes.apply(this, arguments);
			var fileName = this.get('file_meta.name') || "";
			var fileExt = this.get('file_meta.ext') || "";
			if (fileName.indexOf(fileExt) === (fileName.length - fileExt.length)) {
				this.nameIncludesExt = true;
			}
		},
		...
		handleChangePermissions: function () {
			...
			if (permissions.public !== false) {
				if (entities.indexOf("all") === -1) {
					entities.unshift("all");
					this.set('permissions.entities', entities);
				}
			} else {
				var index = entities.indexOf("all");
				if (index !== -1) {
					entities = entities.slice(0, index).concat(entities.slice(index+1));
					this.set('permissions.entities', entities);
				}
			}
		},
		...
		handleChangeEntity: function (entity) {
		...
		getShareLink: function (ttl, options) {
			...
			if (options.short) {
				...
				Drop.Helpers.shortenURL(url, options.callback, {
					obscure: this.get('permissions.public') === false
				});
			} else {
				return url;
			}
		...
		save: function () {
			if (this.id === 'new') {
				...
				var entities = (this.get('permissions.entities') || []).filter(function (e) {
					return e !== "all";
				});
				Drop.client.createPost({
					entity: this.entity,
					type: this.type,
					mentions: entities.map(function (e) {
						return {
							entity: e
						};
					}),
				...
				window.console.error(Error("You need to implement saving existing file with Drop.Models.File.prototype.save"));
			}
		},
