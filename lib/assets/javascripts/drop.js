...
(function () {
	...
	_.extend(Drop, Marbles.Events, Marbles.Accessors, {
		...
		Helpers: {
			...
			avatarURL: function (entity, avatarDigest) {
				if (avatarDigest) {
					...
					return Drop.Helpers.sigilURL(entity, { w: 60 });
				}
			},
		...
		run: function (options) {
			...
			if (this.config.authenticated || Marbles.history.path.match(/^[^a-z]*login[^a-z]/)) {
				Marbles.history.loadURL();
			} else {
				Marbles.history.navigate(this.Helpers.fullPath('/login?redirect='+ Marbles.history.path), { replace: true });
			}
		...
		initAuthButton: function () {
			...
			CupcakeAppsBar.registerHandler('auth', function () {
				if (this.config.authenticated) {
					this.performSignout();
				}
			}.bind(this));
		...
		setAuthTitle: function () {
			CupcakeAppsBar.setItemTitle('auth', this.config.authenticated ? 'Log out' : 'Log in');
		},

		performSignout: function () {
			new Marbles.HTTP({
				method: 'POST',
				url: this.config.SIGNOUT_URL,
